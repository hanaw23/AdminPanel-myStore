...
function ProductDrawer(props) {
  ...
  return (
    <main
      className={` bg-gray-800 fixed  h-full overflow-hidden z-10 bg-opacity-25 inset-0 transform ease-in-out ${
        props.isOpen ? " transition-opacity opacity-100 duration-500 translate-x-0 " : "transition-all delay-500 opacity-0 translate-x-full  "
      }`}
    >
      <section
        className={`bg-gray-300 w-screen max-w-lg right-0 absolute h-full shadow-xl delay-400 duration-500 ease-in-out transition-all transform overflow-y-auto overflow-x-hidden text-gray-700 ${
          props.isOpen ? " translate-x-0 " : " translate-x-full "
        }`}
      >
        <article className="relative w-screen max-w-lg p-8 flex flex-col h-full bg-gray-300">
